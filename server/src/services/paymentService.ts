...
export const paymentService = {
   ...
   */
  async createPaymentSession(amount: number, reservationId: string, customerPhone?: string) {
  ...
  async verifyPayment(sessionId: string) {
    ...
    try {
      const response = await axios.get(`${WAVE_API_BASE_URL}/checkout/sessions/${sessionId}`, {
        headers: {
          'Authorization': `Bearer ${process.env.WAVE_API_KEY}`
        }
      });
